(function () {
  ...
  function validateRecord(raw){
    ...
    const input=r.method==='time'?{method:'time',date:r.source.date,time:r.source.time,rollover:r.source.rollover}:{method:'numbers',numbers:r.source.numbers};
  ...
  function switchMethod(next){if(!['time','numbers'].includes(next))return;method=next;$('method-time').setAttribute('aria-pressed',String(next==='time'));$('method-numbers').setAttribute('aria-pressed',String(next==='numbers'));$('time-fields').hidden=next!=='time';$('number-fields').hidden=next!=='numbers';$('cast-date').required=next==='time';$('cast-time').required=next==='time';['number-upper','number-lower','number-moving'].forEach(id=>{$(id).required=next==='numbers';$(id).disabled=next!=='numbers';});showError('');}
  ...
  function readInput(){return method==='time'?{method:'time',date:$('cast-date').value,time:$('cast-time').value,rollover:$('rollover').value}:{method:'numbers',numbers:[$('number-upper').value,$('number-lower').value,$('number-moving').value].map(Number)};}
  function canReplace(){return !dirty||confirm('這次紀錄有尚未儲存的內容。確定離開這筆紀錄嗎？');}
  function cast(){
    if(!canReplace())return false;
    try{const input=readInput(),result=M.calculate(input),now=new Date().toISOString();current={id:makeId(),input,question:$('question').value.trim(),context:$('context').value.trim(),expected:'',actual:'',status:'待觀察',match:'尚未對照',createdAt:now,updatedAt:now};currentResult=result;dirty=true;formDirty=false;showError('');renderResult();return true;}catch(e){showError(e.message);return false;}
  }
  ...
  function saveCurrent(){
    if(!current)return;
    try{const now=new Date().toISOString(),record=validateRecord({...current,updatedAt:now});let next=records.filter(x=>x.id!==record.id);if(next.length>=500)throw new Error('已達 500 筆紀錄，請先備份並整理舊紀錄。');next.unshift(record);persist(next);current=record;dirty=false;$('save-state').textContent='已儲存在此瀏覽器。';$('save-result').textContent='更新紀錄';$('save-notes').textContent='更新觀察紀錄';$('result').querySelector('.badge').textContent=current.status;notify('紀錄已儲存');}catch(e){notify(e.message||'儲存失敗，請先複製結果或匯出備份。');}
  }
  ...
  function switchView(view){const history=view==='history';$('cast-view').hidden=history;$('history-view').hidden=!history;$('nav-cast').classList.toggle('active',!history);$('nav-history').classList.toggle('active',history);if(history)renderHistory();}
  function openRecord(id){
    if(!canReplace())return false;
    const record=records.find(x=>x.id===id);if(!record)return false;
    current=structuredClone(record);currentResult=M.calculate(record.input);dirty=false;formDirty=false;
    $('question').value=record.question;$('context').value=record.context;switchMethod(record.input.method);
    if(record.input.method==='time'){$('cast-date').value=record.input.date;$('cast-time').value=record.input.time;$('rollover').value=record.input.rollover;updateLunar();}else{['number-upper','number-lower','number-moving'].forEach((id,i)=>$(id).value=record.input.numbers[i]);}
    renderResult();switchView('cast');return true;
  }
  function renderHistory(){
    ...
    $('history-list').innerHTML=records.slice().sort((a,b)=>Date.parse(b.updatedAt)-Date.parse(a.updatedAt)).map(record=>{const r=M.calculate(record.input);const when=r.method==='time'?`${r.source.date} ${r.source.time}`:`三數 ${r.source.numbers.join(' / ')}`;return `<article class="history-item"><span class="history-glyph" aria-hidden="true">${r.base.symbol}</span><div class="history-info"><h3>${escape(shorten(record.question||'未指定問題'))}</h3><p class="meta">${escape(when)} · ${escape(record.status)} · ${escape(record.match)}</p><p class="summary">${r.base.fullName}　${r.movingName}動　之 ${r.changed.fullName}</p></div><div class="actions"><button type="button" class="secondary" data-open="${escape(record.id)}">開啟紀錄</button><button type="button" class="text-button delete-record" data-delete="${escape(record.id)}">刪除</button></div></article>`;}).join('');
    $('history-list').querySelectorAll('[data-open]').forEach(b=>b.addEventListener('click',()=>openRecord(b.dataset.open)));
    $('history-list').querySelectorAll('[data-delete]').forEach(b=>b.addEventListener('click',()=>{const id=b.dataset.delete;if(!confirm('確定刪除這筆紀錄？若需要保留，請先匯出備份。'))return;try{persist(records.filter(x=>x.id!==id));if(current?.id===id){dirty=true;if($('save-state'))$('save-state').textContent='此紀錄已刪除；可再次儲存。';}renderHistory();notify('已刪除紀錄');}catch(e){notify(e.message);}}));
  }
  ...
  $('method-time').addEventListener('click',()=>{switchMethod('time');markFormChanged();});$('method-numbers').addEventListener('click',()=>{switchMethod('numbers');markFormChanged();});
